...
import { sendBookingConfirmation } from "../core/emailService.js"; // ✅ E-Mail-Service importieren
...
export default async function bookingRoutes(fastify, options) {
  ...
  fastify.post("/bookings", async (request, reply) => {
    try {
      ...
      if (user.email && booking.event) {
        try {
          await sendBookingConfirmation(user.email, booking.event);
          console.log("📧 Bestätigungs-E-Mail gesendet an", user.email);
        } catch (mailErr) {
          console.warn("⚠️ E-Mail-Versand fehlgeschlagen:", mailErr.message);
        }
      }
